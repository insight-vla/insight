...
NORM_STATS_FILE="assets/pi05_lego_human_pickplace_flip/maggiewang/lego_human_pickplace_flip_04_03/norm_stats.json"
if [ ! -f "$NORM_STATS_FILE" ]; then
    echo "Computing norm stats..."
    uv run scripts/compute_norm_stats.py --config-name pi05_lego_human_pickplace_flip
else
    echo "Norm stats already exist, skipping..."
fi

# Train on 4x L40S with FSDP, batch 64
uv run scripts/train.py pi05_lego_human_pickplace_flip --exp-name=pi05_human_pickplace_flip --batch-size 64 --fsdp-devices 4

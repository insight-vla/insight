...
#SBATCH --job-name="pi05_drawer_op"
...
NORM_STATS_FILE="assets/pi05_drawer_open_push_combined_04_12/maggiewang/drawer_open_push_combined_04_12/norm_stats.json"
if [ ! -f "$NORM_STATS_FILE" ]; then
    echo "Computing norm stats..."
    uv run scripts/compute_norm_stats.py --config-name pi05_drawer_open_push_combined_04_12
else
    echo "Norm stats already exist, skipping..."
fi

# Train on 2x L40S with FSDP, batch 64
uv run scripts/train.py pi05_drawer_open_push_combined_04_12 --exp-name=pi05_drawer_open_push_combined --batch-size 64 --fsdp-devices 4

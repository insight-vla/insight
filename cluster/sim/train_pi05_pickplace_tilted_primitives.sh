...
#SBATCH --job-name="pi05_pp_tilted"
...
NORM_STATS_FILE="assets/pi05_lego_pickplace_tilted_150_primitives/maggiewang/lego_pickplace_tilted_150_primitives_trimmed/norm_stats.json"
if [ ! -f "$NORM_STATS_FILE" ]; then
    echo "Computing norm stats..."
    uv run scripts/compute_norm_stats.py --config-name pi05_lego_pickplace_tilted_150_primitives
else
    echo "Norm stats already exist, skipping..."
fi

# Train on 2x L40S with FSDP, batch 64
uv run scripts/train.py pi05_lego_pickplace_tilted_150_primitives --exp-name=pi05_pickplace_tilted_150_primitives --batch-size 64 --fsdp-devices 2
